...
class P_controller:
    """
    P 제어 + 정상상태
    출력: state_steady_speed + Kp * error
    """
...
def control_thread_fn(shared: sharedlevel, pump: pump_controller, stop_event: threading.Event):
    p_controller = P_controller(Kp, state_steady_speed, max_pump_speed)
...
def main():
    ...
    t_ctrl = threading.Thread(target=control_thread_fn, args=(shared, pump, stop_event), daemon=True)
