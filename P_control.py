    ...
    try:
        while not stop_event.is_set():
            ...
            if (not valid) or (liquid_height_cm is None) or (age > STALE_SEC):
                ...
                log.add(time.time(), liquid_height_cm, 0, setpoint_cm)  # ✅ 기록
...
def main():
    ...
    t_ctrl = threading.Thread(target=control_thread_fn, args=(shared, pump, stop_event, log), daemon=True)  # ✅ log 전달
